...
int main(int argc, char **argv)
{
    ...
    ifstream iFile("3gcase0_simplex.txt");
    if (!iFile)
    {
    ...
    try {
        ...
        struct timeb rtime;
        ftime(&rtime);		  // 获取时间
        int tbegin = rtime.time; // 开始的秒
        int tmsbegin = rtime.millitm;
        //-------

        mcf->SolveMCF(); // 最小费用流算法

        // time
        ftime(&rtime);
        int out_s = rtime.time - tbegin;
        int out_ms = rtime.millitm - tmsbegin;
        if (out_ms < 0) {
            out_ms += 1000;
            out_s -= 1;
        }
        printf("时间%d %d\n", out_s, out_ms);
        FILE *fil = fopen("time_records.txt", "a");
        fprintf(fil, "%d\n", out_ms);
        fclose(fil);
        ...
        case(MCFClass::kOK) :
            cout << "Optimal Objective Function value = " << mcf->MCFGetFO() << endl; //获取最小费用
            double tu, ts;
            mcf->TimeMCF(tu, ts);
